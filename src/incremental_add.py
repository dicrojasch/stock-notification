...
def validate_existence(ticker, interval="1d"):
    ...
    try:
        ...
        if not cursor.fetchone():
            logger.warning(f"❌ Ticker {ticker} has no initialization records for the {interval} interval.")
            ticker_exists = False
    except sqlite3.OperationalError:
        logger.error("❌ The 'existence' table does not exist in the database.")
        ticker_exists = False
...
def incremental_update(ticker, interval="1d"):
    ...
    try:
        ...
        if last_date_str:
            ...
            if not new_df.empty:
                ...
                if not new_df.empty:
                    ...
                    logger.info(f"➕ {ticker}: {len(new_df)} new rows added.")
                else:
                    logger.info(f"☕ {ticker}: Already up to date.")
            else:
                logger.info(f"☕ {ticker}: No new data since {start_date}.")
        else:
            raise ValueError("Empty table") # Jumps to except for initial download

    except Exception as e:
        logger.info(f"📦 Creating/Resetting table for {ticker} (Reason: {e})...")
        ...
        if not df.empty:
            ...
            logger.info(f"✅ {ticker}: Table created with {len(df)} records.")
